...
> Funding, on autopilot. An AI agent that finds, ranks, and applies to grants & scholarships for you — globally.
...
The end-state product applies on the user's behalf: profile in, AI-drafted application out.

## Status

Front-end prototype. No backend yet — all profile data and saved matches persist in `localStorage`. The discover feed uses sample global opportunities; in production these come from the matching agent.
...
| File | Purpose |
| --- | --- |
| `index.html` | Minimal AI-forward landing + profile type selector |
...
| `saved.html` | Queue of opportunities for AI auto-apply |
...
- [ ] AI matching agent (vector-based eligibility scoring)
- [ ] AI auto-apply agent (form filling + essay drafting)
